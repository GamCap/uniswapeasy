...
export function TestableProvider({
  provider,
  children,
}: PropsWithChildren<{ provider: JsonRpcProvider }>) {
  const connector = useMemo(
    () => initializeWeb3ReactConnector(JsonRpcConnector, { provider }),
    [provider]
  );
  useEffect(() => {
...
export function Provider({
  ...
  useEffect(() => {
    ...
    if (connectors.user) {
      ...
      const eagerConnectors = [
        connectors.metaMask /* connectors.walletConnect*/,
      ];
