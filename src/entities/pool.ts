...
export class Pool {
  ...
  public static getPoolKey(tokenA: Token, tokenB: Token, fee: BigNumberish, tickSpacing: BigNumberish, hooks: string ): PoolKeyStruct {
    return {
  ...
  public constructor(
    ...
    invariant(Number.isInteger(fee.toString()) &&   JSBI.lessThan(JSBI.BigInt(fee.toString()), JSBI.BigInt(1_000_000)), 'FEE')
   ...
   */
  // TODO
  // Partial causes all of these to be possibly undefined
  private async swap(
    zeroForOne: boolean,
    ...
    while (JSBI.notEqual(state.amountSpecifiedRemaining, ZERO) && state.sqrtPriceX96 != sqrtPriceLimitX96) {
      ...
      } else if (step.tickNext && step.tickNext > TickMath.MAX_TICK) {
      ...
      ;[state.sqrtPriceX96, step.amountIn, step.amountOut, step.feeAmount] = SwapMath.computeSwapStep(
        state.sqrtPriceX96,
        (zeroForOne
        ? JSBI.lessThan(step.sqrtPriceNextX96 ?? JSBI.BigInt(0), sqrtPriceLimitX96)
        : JSBI.greaterThan(step.sqrtPriceNextX96 ?? JSBI.BigInt(0), sqrtPriceLimitX96))
          ? sqrtPriceLimitX96
          : step.sqrtPriceNextX96 ?? JSBI.BigInt(0),
        state.liquidity,
      ...
      if (exactInput) {
        state.amountSpecifiedRemaining = JSBI.subtract(
          state.amountSpecifiedRemaining,
          JSBI.add(step.amountIn ?? JSBI.BigInt(0), step.feeAmount ?? JSBI.BigInt(0))
        )
        state.amountCalculated = JSBI.subtract(state.amountCalculated, step.amountOut ?? JSBI.BigInt(0))
      } else {
        state.amountSpecifiedRemaining = JSBI.add(state.amountSpecifiedRemaining, step.amountOut ?? JSBI.BigInt(0))
        state.amountCalculated = JSBI.add(state.amountCalculated, JSBI.add(step.amountIn ?? JSBI.BigInt(0), step.feeAmount ?? JSBI.BigInt(0)))
      }

      
      if (step.sqrtPriceNextX96 && JSBI.equal(state.sqrtPriceX96, step.sqrtPriceNextX96)) {
        ...
        if (step.tickNext) state.tick = zeroForOne ? step.tickNext - 1 : step.tickNext
