...
export abstract class TickList {
  ...
  public static validateList(ticks: Tick[], tickSpacing: BigNumberish) {
    invariant(JSBI.greaterThan(JSBI.BigInt(tickSpacing.toString()), ZERO), 'TICK_SPACING_NONZERO')
    // ensure ticks are spaced appropriately
    invariant(
      ticks.every(({ index }) => JSBI.equal(JSBI.remainder(JSBI.BigInt(index), JSBI.BigInt(tickSpacing.toString())),ZERO)),
      'TICK_SPACING'
    )
